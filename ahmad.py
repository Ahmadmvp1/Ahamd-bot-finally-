...
# ======================
# 🔗 إعدادات API القديم (iva_group) - Fallback
# ======================
IVA_API_URL = "https://maroon-wombat-183778.hostingersite.com/apiivasms/api.php"
IVA_API_KEY = "sk_ef44bbb0c8f04a621c056c44a0d75848869f65f63a2c4ef6d4e727ee9844602c"
IVA_API_USERNAME = "حسابك"
IVA_API_PASSWORD = "الباص"
IVA_API_ENABLED = False  # معطل - نستخدم iVasms فقط
...
USERNAME = "ddama9026@gmail.com"
PASSWORD = "LkPePo77@"
BOT_TOKEN = "8496098542:AAGemKUJwa8_dkedEMV4MFuFQyOdPFaCN3s"
CHAT_IDS = [
    "-1003767104268",   # الجروب
    "-1003786079938",      # القناة
]
REFRESH_INTERVAL = 3
TIMEOUT = 100
MAX_RETRIES = 5
...
def admin_command(message):
    ...
    bot.send_message(user_id, admin_text, parse_mode="HTML", reply_markup=admin_main_menu())
...
def send_welcome(message):
    ...
    fancy_text = (
        "<b>❍<u>𝑨ℎ𝒎𝒂𝒅 • 𝑩𝒐𝒕 </u>❍</b>\n\n"
        "<b>🔋 <u>مـنـور بـوت • Ahmad • يـحـب</u></b>\n\n"
        "<b>🎓 <u>الـمـطـور</u>  • <a href='https://t.me/A7mad_1m'>Ahmad</a></b>\n\n"
        "<b>────────────────────</b>\n"
...
def handle_country_selection(call):
    ...
    # زر الجروب في الأعلى
    markup.add(types.InlineKeyboardButton("اشــتـرڪ هـنـا يـحـب🔱", url="https://t.me/NOII5"))
...
def change_number(call):
    ...
    markup.add(types.InlineKeyboardButton("اشـتـرك هـنـا يـحـب🔱", url="https://t.me/NOII5"))
...
def handle_combo_file(message):
    ...
    try:
        ...
        content = None
        for encoding in ['utf-8', 'utf-16', 'latin-1', 'cp1256', 'iso-8859-6']:
            try:
                ...
                continue
        if content is None:
            bot.reply_to(message, "❌ الملف غير مدعوم! أرسله بصيغة TXT عادية.")
            return
...
# الـ Cookies مأخوذة مباشرة من المتصفح لتجاوز Cloudflare
IVASMS_COOKIES = {
    'ivas_sms_session': '%3D',
    'XSRF-TOKEN': '%3D',
}
...
def login_to_ivasms():
    try:
        ...
        if "login" not in test.url.lower():
            ...
            print(f"[{dash['name']}] ❌ فشل تسجيل الدخول، الـ Cookies منتهية")
            print(f"[{dash['name']}] ⏳ انتظار 5 دقائق...")
            time.sleep(300)
            return False
    except Exception as e:
        print(f"[iVasms] ❌ خطأ في تسجيل الدخول: {e}")
        print(f"[iVasms] ⏳ انتظار 5 دقائق...")
        time.sleep(300)
        return False

def refresh_csrf(session):
    """جلب CSRF token جديد من صفحة SMS"""
    try:
...
def fetch_sms_for_combo_numbers(session, base_url, csrf_token, headers, start_date, end_date):
    """جلب SMS مباشرة لكل رقم في الكومبو"""
    all_messages = []
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT numbers FROM combos")
        rows = c.fetchall()
        conn.close()
        ...
        if not all_numbers:
            print(f"[iVasms] ⚠️ لا توجد أرقام في الكومبو")
            return []

        print(f"[iVasms] 🔍 جلب SMS لـ {len(all_numbers)} رقم مباشرة...")
        sms_url = "https://www.ivasms.com/portal/sms/received/getsms/number/sms"

        # جدد CSRF قبل البدء
        new_csrf = refresh_csrf(session)
        if new_csrf:
            csrf_token = new_csrf
            print(f"[iVasms] 🔑 CSRF جديد: {csrf_token[:20]}...")

        for i, phone in enumerate(all_numbers):
            # جدد CSRF كل 10 أرقام
            if i > 0 and i % 10 == 0:
                new_csrf = refresh_csrf(session)
                if new_csrf:
                    csrf_token = new_csrf
            ...
            try:
                ...
                resp = session.post(sms_url, headers=headers, files={(k, (None, v)) for k, v in payload.items()}, timeout=15)

                # تخطي Page Expired — جدد وجرب مرة ثانية
                if 'Page Expired' in resp.text or resp.status_code == 419:
                    new_csrf = refresh_csrf(session)
                    if new_csrf:
                        csrf_token = new_csrf
                        payload['_token'] = csrf_token
                        resp = session.post(sms_url, headers=headers, data=payload, timeout=15)

                if 'No messages found' in resp.text or len(resp.text) < 50:
                    continue

                print(f"[iVasms] 📄 رد {clean_phone}: {resp.text[:300]}")
                ...
                for card in soup.find_all('div', class_='card-body'):
                    ...
                    if p:
                        sms_texts.append(p.get_text(separator=' ').strip())
                ...
                if not sms_texts:
                    try:
                        ...
                        if isinstance(data, list):
                            for item in data:
                                msg = item.get('message') or item.get('text') or item.get('sms') or str(item)
                                if msg:
                                    sms_texts.append(msg)
                        elif isinstance(data, dict):
                            msg = data.get('message') or data.get('text') or data.get('sms')
                            if msg:
                                sms_texts.append(str(msg))
                    except:
                        pass
                ...
                if not sms_texts:
                    for line in soup.get_text().splitlines():
                        line = line.strip()
                        if len(line) > 15 and any(x in line.lower() for x in ['code', 'otp', 'verify', 'رمز', 'whatsapp', 'telegram']):
                            sms_texts.append(line)

                for sms_text in sms_texts:
                    if sms_text:
                        msg_id = f"{clean_phone}-{sms_text[:50]}"
                        all_messages.append({
                            'id': msg_id,
                            'number': clean_phone,
                            'text': sms_text,
                ...
                print(f"[iVasms] خطأ {phone}: {e}")
                continue

        print(f"[iVasms] ✅ وجد {len(all_messages)} رسالة بالجلب المباشر")
        return all_messages
    except Exception as e:
        print(f"[iVasms] ❌ خطأ في الجلب المباشر: {e}")
        return []

# --- دالة جلب الرسائل من iVasms (محسّنة) ---
def fetch_ivasms_messages():
    ...
    try:
        ...
        def to_multipart(fields):
            """تحويل dict إلى multipart/form-data"""
            return {k: (None, v) for k, v in fields.items()}

        # --- خطوة 1: Get SMS ---
        print(f"[iVasms] 🔴 جلب المجموعات...")
        step1 = session.post(
            "https://www.ivasms.com/portal/sms/received/getsms",
            headers=headers,
            files=to_multipart({'from': start_date, 'to': end_date, '_token': csrf_token}),
            timeout=30
        )

        soup1 = BeautifulSoup(step1.text, 'html.parser')

        # استخراج group_ids من toggleRange
        group_ids = []
        for tag in soup1.find_all(onclick=True):
            match = re.search(r"toggleRange\('([^']+)'", tag.get('onclick', ''))
            if match:
                group_ids.append(match.group(1))

        # أيضاً من الـ JavaScript في الصفحة
        if not group_ids:
            for script in soup1.find_all('script'):
                matches = re.findall(r"toggleRange\('([^']+)'", script.get_text())
                group_ids.extend(matches)

        if not group_ids:
            # استخرج من النص مباشرة
            matches = re.findall(r"toggleRange\('([^']+)'", step1.text)
            group_ids = list(set(matches))

        if not group_ids:
            print(f"[iVasms] ⚠️ ما لاقى مجموعات، استخدام الافتراضي IVORY COAST 9244")
            group_ids = ['IVORY COAST 9244']

        print(f"[iVasms] 📦 المجموعات: {group_ids}")

        for group_id in group_ids:
            # --- خطوة 2: جلب الأرقام للمجموعة ---
            print(f"[iVasms] 📞 جلب أرقام المجموعة {group_id}...")
            step2 = session.post(
                "https://www.ivasms.com/portal/sms/received/getsms/number",
                headers=headers,
                files=to_multipart({'_token': csrf_token, 'start': start_date, 'end': end_date, 'range': group_id}),
                timeout=30
            )

            soup2 = BeautifulSoup(step2.text, 'html.parser')

            # استخراج الأرقام — الموقع يرجع HTML فيه أرقام مع onclick
            phone_numbers = []

            # طريقة 1: onclick يحتوي على الرقم
            for tag in soup2.find_all(onclick=True):
                onclick = tag.get('onclick', '')
                # ابحث عن رقم هاتف في onclick
                match = re.search(r"'(\d{10,15})'", onclick)
                if match:
                    phone_numbers.append(match.group(1))

            # طريقة 2: أي نص فيه رقم هاتف
            if not phone_numbers:
                for tag in soup2.find_all(['div', 'td', 'span']):
                    text = tag.get_text().strip()
                    if re.match(r'^\d{10,15}$', text):
                        phone_numbers.append(text)

            # طريقة 3: regex على النص الكامل
            if not phone_numbers:
                found = re.findall(r'\b(\d{10,15})\b', step2.text)
                phone_numbers = list(set(found))

            # إذا ما لقينا، استخدم أرقام الكومبو
            if not phone_numbers:
                print(f"[iVasms] ℹ️ استخدام أرقام الكومبو للمجموعة {group_id}")
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
                c.execute("SELECT numbers FROM combos")
                rows_db = c.fetchall()
                ...
                for row_db in rows_db:
                    nums = json.loads(row_db[0])
                    phone_numbers.extend([re.sub(r'\D', '', str(n)) for n in nums])

            print(f"[iVasms] 📱 عدد الأرقام في {group_id}: {len(phone_numbers)}")

            for phone in phone_numbers:
                try:
                    # --- خطوة 3: جلب SMS لكل رقم ---
                    step3 = session.post(
                        "https://www.ivasms.com/portal/sms/received/getsms/number/sms",
                        headers=headers,
                        files=to_multipart({'_token': csrf_token, 'start': start_date, 'end': end_date, 'Number': phone, 'Range': group_id}),
                        timeout=15
                    )

                    if 'Page Expired' in step3.text or step3.status_code == 419:
                        new_csrf = refresh_csrf(session)
                        if new_csrf:
                            csrf_token = new_csrf
                            dash['csrf_token'] = csrf_token
                    ...
                    if 'No messages found' in step3.text or len(step3.text.strip()) < 30:
                        continue

                    print(f"[iVasms] 💬 رسالة على {phone}: {step3.text[:200]}")

                    soup3 = BeautifulSoup(step3.text, 'html.parser')

                    # استخراج الرسائل
                    for row in soup3.find_all('tr'):
                        cells = row.find_all('td')
                        if len(cells) >= 2:
                            sms_text = cells[1].get_text(separator=' ').strip() if len(cells) > 1 else ''
                            if not sms_text:
                                sms_text = cells[0].get_text(separator=' ').strip()
                            if sms_text and len(sms_text) > 5:
                                msg_id = f"{phone}-{sms_text[:50]}"
                                all_messages.append({
                                    'id': msg_id,
                                    'number': phone,
                                    'text': sms_text,
                    ...
                    if not all_messages:
                        try:
                            ...
                            for item in items:
                                msg = item.get('message') or item.get('text') or item.get('sms')
                                if msg:
                                    msg_id = f"{phone}-{str(msg)[:50]}"
                                    all_messages.append({
                                        'id': msg_id,
                                        'number': phone,
                                        'text': str(msg),
        ...
        print(f"[iVasms] ✅ إجمالي الرسائل: {len(all_messages)}")
        return all_messages
...
def send_otp_to_user_and_group(date_str, number, sms):
    ...
    if user_id:
        try:
            markup = types.InlineKeyboardMarkup()
            markup.row(
                types.InlineKeyboardButton("𝐴ℎ𝑚𝑎𝑑⚙️", url="https://t.me/A7mad_1m"),
                types.InlineKeyboardButton("𝐾: 𝑆𝑀𝑆", url="https://t.me/-1003767104268")
            )
            bot.send_message(
                user_id,
    ...
    try:
        ...
        keyboard = {
            "inline_keyboard": [
                ...
                [
                    {"text": "💬 𝑆𝑀𝑆 ↗️", "url": "https://t.me/OPT_A5D"},
                    {"text": "𝐵𝑂𝑇 ↗️", "url": "https://t.me/Testing_numburs_bot"}
                ],
                
                # الصف الثالث: زر المطور
                [
                    {"text": "M:𝐵𝑂𝑇 ↗️", "url": "https://t.me/A7mad_1m"}
                ]
            ]
        }
    except Exception as e:
...
# ======================
# 🔗 دالة API القديم (iva_group) - Fallback
# ======================
_iva_api_cache = set()

def fetch_iva_api_messages():
    """جلب الرسائل من API القديم كـ Fallback"""
    if not IVA_API_ENABLED:
        return []
    try:
        params = {
            'api_key': IVA_API_KEY,
            'username': IVA_API_USERNAME,
            'password': IVA_API_PASSWORD,
            '_t': int(time.time() * 1000)
        }
        resp = requests.get(IVA_API_URL, params=params, timeout=25)
        data = resp.json()
        if data.get('success') and data.get('codes'):
            messages = []
            for code in data['codes']:
                number = clean_number(code.get('number', ''))
                sms_text = code.get('sms', '')
                otp = extract_otp(sms_text)
                if otp and number:
                    otp_clean = re.sub(r'\D', '', otp)
                    key = f"{number}|{otp_clean}"
                    if key not in _iva_api_cache:
                        messages.append({
                            'id': key,
                            'number': number,
                            'text': sms_text,
                ...
                if messages:
                    ...
                    for msg in messages:
                        ...
                        if msg_id not in sent_messages:
                            ...
                            # إذا كانت من API القديم، أضفها للكاش
                            if msg.get('_api_key'):
                                _iva_api_cache.add(msg['_api_key'])
                            new_messages += 1
                    ...
                    api_messages = fetch_iva_api_messages()
                    if api_messages:
                        print(f"[API Fallback] 🔗 وجدت {len(api_messages)} رسالة من API القديم")
